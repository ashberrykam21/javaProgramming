...
public class BankAccount {
    ...
    public void deposit(double amount){
        if(amount<=0){
            ...
            return;//exits the method if amount is invalid
        }
        balance +=amount;
    }
    ...
    public void withdraw(double amount){
        ...
        if(amount > balance){// If withdrawing amount  is larger than balance
            System.out.println("Insufficient balance");
            return;
        }
        if(amount<=0){// If withdrawing amount  is zero or  negative
